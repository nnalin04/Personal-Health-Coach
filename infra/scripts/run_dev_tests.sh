...
run_layer "Backend Unit Tests (JUnit5)" \
    bash -c "cd '$ROOT/services/orchestrator' && mvn test -q 2>&1"
...
run_layer "AI Service Tests (pytest + TestClient)" \
    bash -c "cd '$ROOT/services/ai-engine' && python3 -m pip install -q pytest-html && python3 -m pytest tests/ -v --tb=short --html='$AI_REPORT' --self-contained-html 2>&1"
REPORT_LINKS+=("AI Service: $AI_REPORT")

# ── 5. Backend E2E API tests ───────────────────────────────
python3 -m pip install -q -r "$ROOT/infra/tests/requirements.txt" 2>/dev/null
...
run_layer "Backend E2E API Tests (all 33 endpoints)" \
    python3 -m pytest "$ROOT/infra/tests/api/" -v --tb=short \
        --rootdir="$ROOT/infra/tests" \
        -p no:randomly \
        --html="$E2E_REPORT" \
        --self-contained-html
REPORT_LINKS+=("E2E API: $E2E_REPORT")

# ── 6. React Native unit tests (Health OS) ─────────────────
if [[ -d "$ROOT/apps/mobile-app" ]]; then
    run_layer "React Native Unit Tests (Jest)" \
        bash -c "cd '$ROOT/apps/mobile-app' && npx jest --passWithNoTests 2>&1"
fi
